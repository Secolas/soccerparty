...
| **Aftershock** ✅ BUILT | 💥 | UNCOMMON (measured) | When your shot is blocked by the keeper or a wall, a shockwave knocks their nearby outfield players back and the rebound keeps its pace (0.85 floor — a plain head-on save already rebounds at ~0.73, so the proposed 0.6 was a no-op). Keeper never shoved; once per flick. | Counter-Attack, Heavy Ball — the riposte build. Balance: 200/400 pooled = 50.0% [45.1–54.9] vs empty at med — the CPU gets nothing from it because the payoff is POSITIONAL (a cleared goal mouth for the next flick), which the AI never deliberately exploits. Human floor, so ranked UNCOMMON not COMMON; promote on playtest evidence, never demote on this number. |
