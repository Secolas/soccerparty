...
| **Aftershock** ✅ BUILT (v2: stun) | 💥 | RARE (w3) | Your shot carries a charge: the FIRST opponent piece it strikes is SHOCKED until the end of your next flick — a keeper freezes on its spot (still a body: it blocks, but stops tracking/sweeping/magnet-catching), an outfield token loses its tricks (no clearance lunge, no anchor damp, no gridiron roam or punt), and a roaming hazard (dust devil, boulder, crab) stops dead — a charged shot can't even be EATEN by the devil, it zaps it instead. Off a keeper the rebound also keeps pace (0.85 floor vs the natural ~0.73). Once per flick; the stun is LOST if your turn ends before your next flick. | The designed play is a two-flick combo: shock the keeper, catch the rebound off your own token, then shoot at a keeper that cannot move. v1 (blast-back + rebound pace only) measured 50.0% pooled — pure noise, because its payoff was positional and the AI never exploited it. v2 pooled 93/180 = 51.7% (46.7/53.3/55.0) vs empty at med — and this time the CPU DOES benefit passively (a saved flick leaves the keeper frozen for its next flick in the same turn), so the number is a real floor with genuine human upside on top. Ranked RARE (w3): floor near median, deliberate combo above it. Promote on playtest evidence, never demote on this number. |
