    ...
    function drawEco(now){ if(!ecoEnabled()) return;
      ...
      if(ecoPot>0){ ctx.save(); ctx.font="7px 'Press Start 2P', monospace"; ctx.textBaseline='middle'; drawEcoCoin(ctx,W-13,H/2,4.5,Math.floor((now||0)*0.012)); ctx.textAlign='right'; ctx.fillStyle='#ffd84a'; ctx.strokeStyle='rgba(0,0,0,0.7)'; ctx.lineWidth=2; ctx.strokeText(''+ecoPot, W-19, H/2+0.5); ctx.fillText(''+ecoPot, W-19, H/2+0.5); ctx.restore(); } }
