    ...
    function drawSkateTracks(ctx,now){ if(typeof skateTrackPts==='undefined') return; var _st=(typeof hzTier==='function')?hzTier():0;
      ...
      for(var i=0;i<_tks.length;i++){ var pp=_tks[i].pts; if(!pp||pp.length<2) continue; ctx.save(); ctx.lineJoin='round'; ctx.lineCap='round'; _skPath(ctx,pp); ctx.strokeStyle='rgba(28,28,36,0.30)'; ctx.lineWidth=3*COIN_R+4; ctx.stroke(); _skPath(ctx,pp); ctx.strokeStyle='rgba(208,212,222,0.55)'; ctx.lineWidth=3*COIN_R; ctx.stroke(); _skPath(ctx,pp); ctx.strokeStyle='rgba(150,156,168,0.5)'; ctx.lineWidth=3*COIN_R-4; ctx.stroke(); _skPath(ctx,pp); ctx.setLineDash([5,7]); ctx.lineDashOffset=-((now||0)*0.04); ctx.strokeStyle='rgba(255,220,120,0.7)'; ctx.lineWidth=1.5; ctx.stroke(); ctx.setLineDash([]); ctx.restore(); }
