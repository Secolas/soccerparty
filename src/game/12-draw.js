    ...
    function drawSkateHills(ctx,now){ if(typeof skateTrackPts==='undefined') return; if(typeof stadiumHazards==='function' && !stadiumHazards()) return; var _tks=(typeof skateTracks==='function')?skateTracks():skateTrackPts; for(var i=0;i<_tks.length;i++){ var pp=_tks[i].pts; if(!pp||pp.length<2) continue; ctx.save(); ctx.lineJoin='round'; ctx.lineCap='round'; ctx.save(); ctx.translate(0,2); _skPath(ctx,pp); ctx.strokeStyle='rgba(0,0,0,0.16)'; ctx.lineWidth=3*COIN_R+5; ctx.stroke(); ctx.restore(); _skPath(ctx,pp); ctx.strokeStyle='rgba(24,24,32,0.28)'; ctx.lineWidth=3*COIN_R+4; ctx.stroke(); _skPath(ctx,pp); ctx.strokeStyle='rgba(208,212,222,0.40)'; ctx.lineWidth=3*COIN_R; ctx.stroke(); _skPath(ctx,pp); ctx.strokeStyle='rgba(150,156,168,0.30)'; ctx.lineWidth=3*COIN_R-4; ctx.stroke(); _skPath(ctx,pp); ctx.strokeStyle='rgba(255,255,255,0.14)'; ctx.lineWidth=2; ctx.stroke(); _skPath(ctx,pp); ctx.setLineDash([5,7]); ctx.lineDashOffset=0; ctx.strokeStyle='rgba(255,220,120,0.55)'; ctx.lineWidth=1.5; ctx.stroke(); ctx.setLineDash([]); ctx.restore(); } if(typeof skateTube!=='undefined' && skateTube && typeof phase!=='undefined' && phase==='play' && typeof coin!=='undefined' && typeof drawBall==='function'){ try{ drawBall(coin.x,coin.y); }catch(e){} } }
