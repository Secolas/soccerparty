    ...
    function loop(ts){
      ...
      maybeAI(delta); if(pen&&pen.active) penTick(); try{ royFlipperTick(delta); }catch(e){} try{ royGeyserTick(delta); }catch(e){} try{ royDevilTick(delta); }catch(e){} if(royBlizzard()){ var _ps=Math.sin(royGustPhase); royGustPhase+=ROY_GUST_FREQ; if(_ps<=0 && Math.sin(royGustPhase)>0) royGustDir=Math.random()*6.2832; } try{ nsCam(delta); }catch(e){}
      draw(ts);
