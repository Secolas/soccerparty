    ...
    function drawNumBoxes(ctx,now){ if(typeof numBoxes==='undefined'||!numBoxes.length) return; var flash=(typeof rouletteFlash!=='undefined')?rouletteFlash:0, win=(typeof rouletteBox!=='undefined')?rouletteBox:null; ctx.save(); ctx.textAlign='center'; ctx.textBaseline='middle'; ctx.font='bold 24px monospace'; for(var i=0;i<numBoxes.length;i++){ var b=numBoxes[i], red=((b.r+b.c)%2===0), x=b.x-b.w/2, y=b.y-b.h/2; ctx.fillStyle=red?'rgba(166,28,38,0.62)':'rgba(14,14,18,0.66)'; ctx.fillRect(x,y,b.w,b.h); var isW=(win===b&&flash>0); if(isW){ ctx.save(); ctx.globalAlpha=Math.min(0.6,flash/26); ctx.fillStyle='#ffe066'; ctx.fillRect(x,y,b.w,b.h); ctx.restore(); } ctx.strokeStyle='rgba(255,232,150,0.42)'; ctx.lineWidth=1; ctx.strokeRect(x,y,b.w,b.h); ctx.fillStyle='rgba(0,0,0,0.55)'; ctx.fillText(''+b.n,b.x+0.7,b.y+0.7); ctx.fillStyle=isW?'#3a2a00':'rgba(255,250,235,0.9)'; ctx.fillText(''+b.n,b.x,b.y); } ctx.restore(); }
