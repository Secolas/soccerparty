    ...
    // crest light: a central band that glows brightest in the middle (raised top of the hill) and fades dark at the low ends
    function _skCrest(ctx,pp){ ctx.save(); ctx.lineCap='round'; ctx.lineJoin='round'; var n=pp.length-1; for(var i=0;i<n;i++){ var pr=(i+0.5)/n, a=0.03+0.30*Math.sin(pr*Math.PI); ctx.strokeStyle='rgba(255,255,255,'+a.toFixed(3)+')'; ctx.lineWidth=(3*COIN_R)*0.44; ctx.beginPath(); ctx.moveTo(pp[i].x,pp[i].y); ctx.lineTo(pp[i+1].x,pp[i+1].y); ctx.stroke(); } ctx.restore(); }
    // cross ribs: ramp "treads" across the track (dark groove + light lip) to read as a sloped surface
    function _skRibs(ctx,pp){ ctx.save(); ctx.lineCap='round'; var hw=(3*COIN_R)/2-1.5; for(var i=2;i<pp.length-1;i+=2){ var tx=pp[i+1].x-pp[i-1].x, ty=pp[i+1].y-pp[i-1].y, tl=Math.hypot(tx,ty)||1, ux=tx/tl, uy=ty/tl, nx=-uy, ny=ux, cx=pp[i].x, cy=pp[i].y; ctx.strokeStyle='rgba(34,38,46,0.45)'; ctx.lineWidth=1.6; ctx.beginPath(); ctx.moveTo(cx-nx*hw,cy-ny*hw); ctx.lineTo(cx+nx*hw,cy+ny*hw); ctx.stroke(); ctx.strokeStyle='rgba(220,224,234,0.28)'; ctx.lineWidth=0.8; ctx.beginPath(); ctx.moveTo(cx-nx*hw+ux*1.3,cy-ny*hw+uy*1.3); ctx.lineTo(cx+nx*hw+ux*1.3,cy+ny*hw+uy*1.3); ctx.stroke(); } ctx.restore(); }
