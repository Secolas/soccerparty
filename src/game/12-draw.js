    ...
    function drawSkateTracks(ctx,now){ if(typeof skateTrackPts==='undefined') return; var _st=(typeof hzTier==='function')?hzTier():0;
      // med+: curved half-pipe transition walls on the sides (the arch you skate up)
      if(_st>=1){ var bw=Math.min(34,(W-WALL*2)*0.14); for(var side=0;side<2;side++){ var lft=(side===0), x=lft?WALL:(W-WALL-bw); ctx.save(); var g=ctx.createLinearGradient(lft?WALL:(W-WALL),0,lft?(WALL+bw):(W-WALL-bw),0); g.addColorStop(0,'rgba(255,255,255,0.16)'); g.addColorStop(0.5,'rgba(190,196,206,0.07)'); g.addColorStop(1,'rgba(0,0,0,0.20)'); ctx.fillStyle=g; ctx.fillRect(x,WALL,bw,H-WALL*2); ctx.strokeStyle='rgba(255,255,255,0.22)'; ctx.lineWidth=1; for(var c=1;c<=3;c++){ var frac=c/4; ctx.beginPath(); for(var yy=WALL;yy<=H-WALL;yy+=8){ var bow=Math.sin((yy-WALL)/((H-WALL*2)||1)*Math.PI)*3; var lx=lft?(WALL+bw*frac+bow):(W-WALL-bw*frac-bow); if(yy===WALL) ctx.moveTo(lx,yy); else ctx.lineTo(lx,yy); } ctx.stroke(); } ctx.strokeStyle='rgba(255,246,220,0.55)'; ctx.lineWidth=2; ctx.beginPath(); for(var y2=WALL;y2<=H-WALL;y2+=6){ var bow2=Math.sin((y2-WALL)/((H-WALL*2)||1)*Math.PI)*4; var cx2=lft?(WALL+bw+bow2):(W-WALL-bw-bow2); if(y2===WALL) ctx.moveTo(cx2,y2); else ctx.lineTo(cx2,y2); } ctx.stroke(); ctx.restore(); } }
      // tracks (all tiers): coin-wide concrete channels following the S/curves
      var _tks=(typeof skateTracks==='function')?skateTracks():skateTrackPts;
      for(var i=0;i<_tks.length;i++){ var pp=_tks[i].pts; if(!pp||pp.length<2) continue; ctx.save(); ctx.lineJoin='round'; ctx.lineCap='round'; _skPath(ctx,pp); ctx.strokeStyle='rgba(28,28,36,0.30)'; ctx.lineWidth=2*COIN_R+4; ctx.stroke(); _skPath(ctx,pp); ctx.strokeStyle='rgba(208,212,222,0.55)'; ctx.lineWidth=2*COIN_R; ctx.stroke(); _skPath(ctx,pp); ctx.strokeStyle='rgba(150,156,168,0.5)'; ctx.lineWidth=2*COIN_R-4; ctx.stroke(); _skPath(ctx,pp); ctx.setLineDash([5,7]); ctx.lineDashOffset=-((now||0)*0.04); ctx.strokeStyle='rgba(255,220,120,0.7)'; ctx.lineWidth=1.5; ctx.stroke(); ctx.setLineDash([]); ctx.restore(); }
      // hard: dash-shaped kicker humps — pitch-blended, inflated
      if(_st>=2){ var _bls=(typeof skateBumps==='function')?skateBumps():skateBumpList; for(var b=0;b<_bls.length;b++){ var p=_bls[b]; ctx.save(); ctx.translate(p.x,p.y); var w=p.w,h=p.h; ctx.strokeStyle='rgba(0,0,0,0.20)'; ctx.lineWidth=2; _skBar(ctx,w,h); ctx.stroke(); var g2=ctx.createLinearGradient(0,-h,0,h); g2.addColorStop(0,'rgba(255,255,255,0.26)'); g2.addColorStop(0.5,'rgba(255,255,255,0.05)'); g2.addColorStop(1,'rgba(0,0,0,0.18)'); ctx.fillStyle=g2; _skBar(ctx,w,h); ctx.fill(); ctx.fillStyle='rgba(255,255,255,0.24)'; _skBar(ctx,w*0.7,h*0.4); ctx.fill(); ctx.restore(); } } }
