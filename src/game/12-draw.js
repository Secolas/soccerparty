    ...
    function drawSkateRails(ctx,now){ if(typeof skateRails!=='function') return; var rls=skateRails(); for(var i=0;i<rls.length;i++){ var y=rls[i]; ctx.save(); ctx.strokeStyle='rgba(235,205,120,0.85)'; ctx.lineWidth=3; ctx.beginPath(); ctx.moveTo(WALL+6,y); ctx.lineTo(W-WALL-6,y); ctx.stroke(); ctx.strokeStyle='rgba(120,90,40,0.6)'; ctx.lineWidth=1; ctx.beginPath(); ctx.moveTo(WALL+6,y+2); ctx.lineTo(W-WALL-6,y+2); ctx.stroke(); ctx.fillStyle='rgba(90,70,40,0.85)'; for(var x=WALL+16;x<W-WALL-10;x+=34){ ctx.fillRect(x-1,y-3,2,6); } ctx.restore(); } }
