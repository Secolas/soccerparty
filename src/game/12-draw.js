    ...
    function _skCrest(ctx,pp){ ctx.save(); ctx.lineCap='round'; ctx.lineJoin='round'; var n=pp.length-1; for(var i=0;i<n;i++){ var pr=(i+0.5)/n, a=0.14*Math.sin(pr*Math.PI); if(a<0.01) continue; ctx.strokeStyle='rgba(236,240,248,'+a.toFixed(3)+')'; ctx.lineWidth=3*COIN_R-3; ctx.beginPath(); ctx.moveTo(pp[i].x,pp[i].y); ctx.lineTo(pp[i+1].x,pp[i+1].y); ctx.stroke(); } ctx.restore(); }
    ...
    function _skMouth(ctx,ex,ey,nbx,nby){ var ox=ex-nbx, oy=ey-nby, ol=Math.hypot(ox,oy)||1; ox/=ol; oy/=ol; var w=3*COIN_R; ctx.save(); ctx.translate(ex,ey); ctx.rotate(Math.atan2(oy,ox)); var g=ctx.createLinearGradient(-5,0,15,0); g.addColorStop(0,'rgba(126,130,140,1)'); g.addColorStop(0.5,'rgba(120,124,134,0.55)'); g.addColorStop(1,'rgba(120,124,134,0)'); ctx.fillStyle=g; ctx.beginPath(); ctx.moveTo(-6,-w/2-2); ctx.lineTo(15,-w*0.30); ctx.lineTo(15,w*0.30); ctx.lineTo(-6,w/2+2); ctx.closePath(); ctx.fill(); ctx.strokeStyle='rgba(255,255,255,0.12)'; ctx.lineWidth=1; ctx.beginPath(); ctx.moveTo(-4,-w/2); ctx.lineTo(13,-w*0.30); ctx.moveTo(-4,w/2); ctx.lineTo(13,w*0.30); ctx.stroke(); ctx.restore(); }
    // aim-guide helper: does point (x,y) hit a solid track wall? (open near the front mouths)
    function _skTrackHit(x,y){ if(!((typeof boardKey!=='undefined')&&boardKey==='skate'&&(typeof stadiumHazards==='function')&&stadiumHazards())) return null; if(typeof skateTrackPts==='undefined'||!skateTrackPts.length) return null; var _HW=1.5*COIN_R; for(var _t=0;_t<skateTrackPts.length;_t++){ var _pp=skateTrackPts[_t].pts, _lst=_pp.length-1, _bd=1e9,_bpx=0,_bpy=0; for(var _q=0;_q<_lst;_q++){ var _ax=_pp[_q].x,_ay=_pp[_q].y,_ex=_pp[_q+1].x-_ax,_ey=_pp[_q+1].y-_ay,_l2=(_ex*_ex+_ey*_ey)||1,_tp=Math.max(0,Math.min(1,((x-_ax)*_ex+(y-_ay)*_ey)/_l2)),_px=_ax+_ex*_tp,_py=_ay+_ey*_tp,_dd=Math.hypot(x-_px,y-_py); if(_dd<_bd){_bd=_dd;_bpx=_px;_bpy=_py;} } if(_bd>0.01 && _bd<_HW+COIN_R){ return {nx:(x-_bpx)/_bd, ny:(y-_bpy)/_bd, pen:(_HW+COIN_R)-_bd}; } } return null; }
    function drawSkateHills(ctx,now){ if(typeof skateTrackPts==='undefined') return; if(typeof stadiumHazards==='function' && !stadiumHazards()) return; var _tks=(typeof skateTracks==='function')?skateTracks():skateTrackPts; for(var i=0;i<_tks.length;i++){ var pp=_tks[i].pts; if(!pp||pp.length<2) continue; ctx.save(); ctx.lineJoin='round'; ctx.lineCap='round'; ctx.save(); ctx.strokeStyle='rgba(0,0,0,0.20)'; ctx.lineWidth=3*COIN_R+5; ctx.beginPath(); for(var _si=0;_si<pp.length;_si++){ var _pr=_si/(pp.length-1), _off=1.5+4.5*Math.sin(_pr*Math.PI); if(_si===0) ctx.moveTo(pp[_si].x,pp[_si].y+_off); else ctx.lineTo(pp[_si].x,pp[_si].y+_off); } ctx.stroke(); ctx.restore(); _skPath(ctx,pp); ctx.strokeStyle='rgba(40,44,52,0.9)'; ctx.lineWidth=3*COIN_R+4; ctx.stroke(); _skPath(ctx,pp); ctx.strokeStyle='rgba(118,122,132,1)'; ctx.lineWidth=3*COIN_R; ctx.stroke(); try{ _skCrest(ctx,pp); }catch(e){} _skPath(ctx,pp); ctx.setLineDash([5,7]); ctx.lineDashOffset=0; ctx.strokeStyle='rgba(255,220,120,0.7)'; ctx.lineWidth=1.5; ctx.stroke(); ctx.setLineDash([]); ctx.restore(); try{ _skMouth(ctx,pp[0].x,pp[0].y,pp[1].x,pp[1].y); _skMouth(ctx,pp[pp.length-1].x,pp[pp.length-1].y,pp[pp.length-2].x,pp[pp.length-2].y); }catch(e){} } var _onTrack=(typeof skateTube!=='undefined' && skateTube); if(!_onTrack && typeof coin!=='undefined'){ for(var _t=0;_t<_tks.length && !_onTrack;_t++){ var _tp3=_tks[_t].pts; if(!_tp3) continue; for(var _q3=0;_q3<_tp3.length-1;_q3++){ var _ax3=_tp3[_q3].x,_ay3=_tp3[_q3].y,_ex3=_tp3[_q3+1].x-_ax3,_ey3=_tp3[_q3+1].y-_ay3,_l3=(_ex3*_ex3+_ey3*_ey3)||1,_pr3=Math.max(0,Math.min(1,((coin.x-_ax3)*_ex3+(coin.y-_ay3)*_ey3)/_l3)),_px3=_ax3+_ex3*_pr3,_py3=_ay3+_ey3*_pr3; if(Math.hypot(coin.x-_px3,coin.y-_py3)<3*COIN_R){ _onTrack=true; break; } } } } if(_onTrack && typeof phase!=='undefined' && phase==='play' && typeof coin!=='undefined' && typeof drawBall==='function'){ try{ ctx.save(); ctx.globalAlpha=0.28; ctx.fillStyle='#000'; ctx.beginPath(); ctx.ellipse(coin.x,coin.y+COIN_R*0.9,COIN_R*0.9,COIN_R*0.45,0,0,Math.PI*2); ctx.fill(); ctx.restore(); drawBall(coin.x,coin.y); }catch(e){} } }
    function drawJungleBananas(ctx,now){ if(typeof jungleBananas==='undefined') return; for(var i=0;i<jungleBananas.length;i++){ var b=jungleBananas[i]; ctx.save(); ctx.translate(b.x,b.y); ctx.rotate(b.a); ctx.fillStyle='rgba(0,0,0,0.22)'; ctx.beginPath(); ctx.ellipse(1,2,b.r+1,b.r*0.7,0,0,6.283); ctx.fill(); ctx.lineCap='round'; ctx.strokeStyle='#c99a12'; ctx.lineWidth=3.4; ctx.beginPath(); ctx.arc(0,0,b.r,0.5,Math.PI-0.5); ctx.stroke(); ctx.strokeStyle='#f2d848'; ctx.lineWidth=1.8; ctx.beginPath(); ctx.arc(0,-0.5,b.r,0.6,Math.PI-0.6); ctx.stroke(); ctx.fillStyle='#6e5010'; ctx.beginPath(); ctx.arc(Math.cos(Math.PI-0.5)*b.r,Math.sin(Math.PI-0.5)*b.r,1.5,0,6.283); ctx.fill(); ctx.beginPath(); ctx.arc(Math.cos(0.5)*b.r,Math.sin(0.5)*b.r,1.5,0,6.283); ctx.fill(); ctx.restore(); } }
    function drawJungleLogs(ctx,now){ if(typeof jungleLogs==='undefined') return; for(var i=0;i<jungleLogs.length;i++){ var l=jungleLogs[i]; ctx.save(); ctx.translate(l.x,l.y); ctx.fillStyle='rgba(0,0,0,0.30)'; ctx.beginPath(); ctx.ellipse(2.5,1,l.r+2,l.hl+2,0,0,6.283); ctx.fill(); var g=ctx.createLinearGradient(-l.r,0,l.r,0); g.addColorStop(0,'#4a2f18'); g.addColorStop(0.45,'#8a5a2e'); g.addColorStop(1,'#5a3a1e'); ctx.fillStyle=g; _skBar(ctx,l.r*2,l.hl*2); ctx.fill(); ctx.strokeStyle='#331f0e'; ctx.lineWidth=1.2; _skBar(ctx,l.r*2,l.hl*2); ctx.stroke(); ctx.strokeStyle='rgba(40,26,14,0.6)'; ctx.lineWidth=1; var _ro=((l.roll%8)+8)%8; for(var k=-3;k<=3;k++){ var ry=k*8+_ro; if(ry<-l.hl+1||ry>l.hl-1) continue; ctx.beginPath(); ctx.moveTo(-l.r+1,ry); ctx.lineTo(l.r-1,ry); ctx.stroke(); } ctx.fillStyle='#c39a5a'; ctx.beginPath(); ctx.ellipse(0,-l.hl+1,l.r-0.5,l.r*0.55,0,0,6.283); ctx.fill(); ctx.strokeStyle='#7a5222'; ctx.lineWidth=0.8; ctx.beginPath(); ctx.arc(0,-l.hl+1,l.r*0.5,0,6.283); ctx.stroke(); ctx.restore(); } }
