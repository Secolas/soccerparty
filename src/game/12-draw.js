    ...
    // jelly trampoline pads on the candy pitch (positions come from candyPads() in physics)
    function drawCandyPads(ctx,now){ var pads=candyPads(); var cols=['#ff8ac0','#8ae0d0','#ffd86a','#c0a0ff','#8ad0ff']; for(var i=0;i<pads.length;i++){ var p=pads[i], pu=1+0.12*Math.sin(now*0.006+i*1.7), r=CANDY_PAD_R; ctx.save(); ctx.translate(p.x,p.y); ctx.globalAlpha=0.85; ctx.fillStyle=cols[i%5]; ctx.beginPath(); ctx.ellipse(0,0,r*pu,r*0.7*pu,0,0,6.283); ctx.fill(); ctx.globalAlpha=0.5; ctx.fillStyle='#ffffff'; ctx.beginPath(); ctx.ellipse(-r*0.3,-r*0.25,r*0.3,r*0.18,0,0,6.283); ctx.fill(); ctx.globalAlpha=0.9; ctx.strokeStyle='rgba(255,255,255,0.6)'; ctx.lineWidth=1; ctx.beginPath(); ctx.ellipse(0,0,r*pu,r*0.7*pu,0,0,6.283); ctx.stroke(); ctx.restore(); } ctx.globalAlpha=1; }
    ...
    function drawRouletteWheel(ctx,now){ var cx=W/2,cy=H/2,R=ROUL_R, ang=(typeof rouletteAng!=='undefined')?rouletteAng:0; var cols=['#c83a4a','#2a2a30','#d6b054','#3a6ad0','#3aa050','#c86ad0']; var flash=(typeof rouletteFlash!=='undefined')?rouletteFlash:0; ctx.save(); ctx.translate(cx,cy); ctx.rotate(ang); for(var s=0;s<6;s++){ var a0=s/6*6.283,a1=(s+1)/6*6.283; ctx.fillStyle=cols[s]; ctx.beginPath(); ctx.moveTo(0,0); ctx.arc(0,0,R,a0,a1); ctx.closePath(); ctx.fill(); if(flash>0&&(s+1)===rouletteLastN){ ctx.save(); ctx.globalAlpha=Math.min(0.85,flash/26); ctx.fillStyle='#ffffff'; ctx.beginPath(); ctx.moveTo(0,0); ctx.arc(0,0,R,a0,a1); ctx.closePath(); ctx.fill(); ctx.restore(); } ctx.save(); ctx.rotate((s+0.5)/6*6.283); var _fs=Math.max(11,Math.round(R*0.46)); ctx.font='bold '+_fs+'px monospace'; ctx.textAlign='center'; ctx.textBaseline='middle'; ctx.fillStyle='rgba(0,0,0,0.7)'; ctx.fillText(''+(s+1),0.6,-R*0.64+0.6); ctx.fillStyle='rgba(255,252,232,1)'; ctx.fillText(''+(s+1),0,-R*0.64); ctx.restore(); } ctx.restore(); ctx.strokeStyle='rgba(255,235,170,0.85)'; ctx.lineWidth=2; ctx.beginPath(); ctx.arc(cx,cy,R,0,6.283); ctx.stroke(); ctx.fillStyle='#3a2f18'; ctx.beginPath(); ctx.arc(cx,cy,R*0.12,0,6.283); ctx.fill(); ctx.fillStyle='#ffe9a8'; ctx.beginPath(); ctx.arc(cx,cy,R*0.055,0,6.283); ctx.fill(); ctx.fillStyle='#ffe9a8'; ctx.beginPath(); ctx.moveTo(cx,cy-R-1); ctx.lineTo(cx-3,cy-R-6); ctx.lineTo(cx+3,cy-R-6); ctx.closePath(); ctx.fill(); if(flash>0){ ctx.save(); ctx.globalAlpha=Math.min(1,flash/22); ctx.font='bold 16px monospace'; ctx.textAlign='center'; ctx.textBaseline='middle'; ctx.strokeStyle='rgba(0,0,0,0.75)'; ctx.lineWidth=3; ctx.strokeText(''+rouletteLastN,cx,cy-R-16); ctx.fillStyle='#fff'; ctx.fillText(''+rouletteLastN,cx,cy-R-16); ctx.restore(); } }
