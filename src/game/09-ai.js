    ...
    function aiFlick(){
      ...
      if(pen&&pen.active){ var _Z=['L','C','R']; var _kp=pen.dive; var _sp=aiLevel==='hard'?0.9:(aiLevel==='med'?0.6:0.3); var _pick; if(Math.random()<_sp){ var _aw=[]; for(var _i=0;_i<3;_i++){ if(_Z[_i]!==_kp) _aw.push(_Z[_i]); } _pick=_aw[Math.floor(Math.random()*_aw.length)]; } else { _pick=_Z[Math.floor(Math.random()*3)]; } var _off=(GOAL_W/2)*(aiLevel==='hard'?0.9:(aiLevel==='med'?0.78:0.5)); var _bx=W/2+(_pick==='L'?-_off:(_pick==='R'?_off:0)); var _err=aiLevel==='hard'?4:(aiLevel==='med'?9:16); var _gx=_bx+(Math.random()*2-1)*_err; var _gy=t==='red'?NET_DEPTH+COIN_R+1:H-NET_DEPTH-COIN_R-1; var _dx=_gx-coin.x,_dy=_gy-coin.y,_di=Math.hypot(_dx,_dy),_ang=Math.atan2(_dy,_dx); var _spd=Math.min(5.1,Math.max(4.2,_di*0.05+3.0)); coin.vx=Math.cos(_ang)*_spd; coin.vy=Math.sin(_ang)*_spd; flickCount++; _achBounces=0; hitOwn=false; moving=true; ghostUsed=false; ghosting=false; portalUsed=false; ricochetUsed=false; serpentPhase=0; turnFlash=Math.max(turnFlash,10); return; }
      ...
      hitOwn=false; moving=true; ghostUsed=false; ghosting=false; portalUsed=false; ricochetUsed=false; _aiChipRoll=0; _camShotDone=false; serpentPhase=0; serpentBase=Math.atan2(coin.vy,coin.vx); serpentDir=1; wetBase=Math.atan2(coin.vy,coin.vx); wetPhase=0; drillUsed=false; (function(){ var _bsp=Math.hypot(coin.vx,coin.vy)||1; backspinFx=coin.vx/_bsp; backspinFy=coin.vy/_bsp; backspinPhase=0; })(); steerBudget=(TAC.guided?40:0); steerHold=null; try{ ecoFlickStart(); }catch(e){} try{ trioReset(); }catch(e){} turnFlash=Math.max(turnFlash,10);
