    ...
    function segRow(host,label,opts,getVal,setVal,cols,lockFn,doneFn){
      ...
      function restyle(){ btns.forEach(([b,v,l])=>{ var lk=!!_lr(v); var dn=(typeof doneFn==='function'&&doneFn(v)); const s=getVal()===v&&!lk; b.textContent=(lk?'🔒 ':(dn?'✓ ':''))+l; b.style.background=s?'#a9c94b':'#0e0a16'; b.style.color=s?'#0b0910':(lk?'#5a5070':(dn?'#a9c94b':'#8a7ea0')); b.style.borderColor=s?'#a9c94b':(lk?'#2a2438':(dn?'#3a5a2a':'#3a3050')); b.style.opacity=lk?'0.6':'1'; }); }
      opts.forEach(([v,l])=>{ const b=mk('button',SQBTN+'font-size:7px;padding:4px;text-align:center;',l); b.onclick=()=>{ var lr=_lr(v); if(lr){ try{ _lockMsg(lr); }catch(e){} return; } setVal(v); restyle(); }; btns.push([b,v,l]); row.appendChild(b); });
    ...
    var _modeCfg={ exhibition:null, tournament:null }; function _cfgSeed(m){ var sz=(m==='tournament')?5:((typeof teamSize!=='undefined')?teamSize:5); return { size:sz, red:(formationName&&formationName.red)||defaultFormation(sz), blue:(formationName&&formationName.blue)||defaultFormation(sz) }; } function _cfgApply(m){ if(m!=='exhibition'&&m!=='tournament') return; var c=_modeCfg[m]||(_modeCfg[m]=_cfgSeed(m)); teamSize=c.size; formationName={red:c.red,blue:c.blue}; } function _cfgSave(m){ if(m!=='exhibition'&&m!=='tournament') return; _modeCfg[m]={ size:teamSize, red:(formationName&&formationName.red)||defaultFormation(teamSize), blue:(formationName&&formationName.blue)||defaultFormation(teamSize) }; try{ spSaveSettings(); }catch(e){} } function buildPre(){ pen=null; try{ var _me0=document.getElementById('ns_matchend'); if(_me0&&_me0.parentNode) _me0.parentNode.removeChild(_me0); }catch(e){}
      ...
      tPrac.onclick=()=>{ if(!_firstDone()){ _lockMsg(); return; } mode='practice'; styleTabs(); renderBody(); }; function openSettings(){ try{ var ex=document.getElementById('ns_settings'); if(ex){ ex.remove(); return; } var ov=mk('div','position:fixed;inset:0;z-index:100000;background:rgba(7,6,12,0.975);overflow-y:auto;'); ov.id='ns_settings'; var spad=mk('div','padding:16px 14px 26px;max-width:460px;margin:0 auto;box-sizing:border-box;'); var shead=mk('div','display:flex;align-items:center;justify-content:space-between;margin-bottom:14px;'); shead.appendChild(mk('div',FS(11,'#a9c94b')+'letter-spacing:1px;','SETTINGS')); var sx=mk('button',FS(12,'#0b0910')+'background:#a9c94b;border:none;border-radius:6px;width:30px;height:30px;cursor:pointer;','✕'); sx.onclick=function(){ ov.remove(); }; shead.appendChild(sx); spad.appendChild(shead); function _srow(label,desc,getv,setv){ var r=mk('div','display:flex;align-items:center;gap:11px;padding:11px;margin-bottom:8px;border-radius:8px;border:2px solid #4a3a5e;background:linear-gradient(#1a1330,#0e0a18);'); var tx=mk('div','flex:1;min-width:0;'); tx.appendChild(mk('div',FS(9,'#f4e9c8')+'margin-bottom:3px;',label)); if(desc) tx.appendChild(mk('div','font-size:10px;line-height:1.5;color:#9a8fb0;',desc)); r.appendChild(tx); var pill=mk('button',FS(8,'#0b1a0e')+'border:none;border-radius:7px;padding:9px 13px;min-width:52px;cursor:pointer;letter-spacing:1px;',''); function _paint(){ var on=!!getv(); pill.textContent=on?'ON':'OFF'; pill.style.background=on?'#a9c94b':'#3a3050'; pill.style.color=on?'#0b1a0e':'#c7bcd8'; } pill.onclick=function(){ setv(!getv()); _paint(); try{spSaveSettings();}catch(e){} try{ if(!muted) sfxClick(); }catch(e){} }; _paint(); r.appendChild(pill); spad.appendChild(r); } _srow('SOUND','Game sound effects.',function(){return !muted;},function(v){ muted=!v; var _sb=el('ns_sound'); if(_sb) _sb.textContent='SOUND: '+(muted?'OFF':'ON'); if(!muted){ try{audio(); sfxTurn();}catch(e){} } else { try{stopAnthem();}catch(e){} } }); _srow('MUSIC','Team anthems and menu music.',function(){return musicOn;},function(v){ musicOn=v; var _mb=el('ns_music'); if(_mb) _mb.textContent='MUSIC: '+(musicOn?'ON':'OFF'); if(!musicOn){ try{stopAnthem();}catch(e){} } }); _srow('HAPTICS','Vibration feedback on your device.',function(){return hapticsOn;},function(v){ hapticsOn=v; var _hb=el('ns_haptics'); if(_hb) _hb.textContent='HAPTICS: '+(hapticsOn?'ON':'OFF'); if(hapticsOn){ try{ if(typeof navigator!=='undefined'&&navigator.vibrate) navigator.vibrate([25,40,60]); }catch(e){} } }); _srow('REDUCE MOTION','Calm the screen shake, camera zoom and pulsing effects.',function(){return reduceMotion;},function(v){ reduceMotion=v; try{applyReduceMotion();}catch(e){} }); ov.appendChild(spad); (document.body||document.documentElement).appendChild(ov); }catch(e){} }
      const tSettings=mk('button',SQBTN+'padding:4px 8px;text-align:center;display:flex;align-items:center;justify-content:center;',''); (function(){ var N=15, _gc=document.createElement('canvas'); _gc.width=N; _gc.height=N; _gc.style.cssText='width:18px;height:18px;image-rendering:pixelated;display:block;'; try{ var _g=_gc.getContext('2d'); _g.imageSmoothingEnabled=false; var C=(N-1)/2, BODY=5.0, HOLE=1.9, TEETH=8, TR=5.7, TW=1.5, x, y; function _sol(px,py){ var dx=px-C,dy=py-C; if(Math.sqrt(dx*dx+dy*dy)<=BODY) return true; for(var k=0;k<TEETH;k++){ var a=k*(Math.PI*2/TEETH), tx=C+Math.cos(a)*TR, ty=C+Math.sin(a)*TR; if(Math.abs(px-tx)<=TW&&Math.abs(py-ty)<=TW) return true; } return false; } function _hol(px,py){ var dx=px-C,dy=py-C; return Math.sqrt(dx*dx+dy*dy)<=HOLE; } for(y=0;y<N;y++)for(x=0;x<N;x++){ if(_sol(x,y)) continue; if(_sol(x-1,y)||_sol(x+1,y)||_sol(x,y-1)||_sol(x,y+1)||_sol(x-1,y-1)||_sol(x+1,y-1)||_sol(x-1,y+1)||_sol(x+1,y+1)){ _g.fillStyle='#100c18'; _g.fillRect(x,y,1,1); } } for(y=0;y<N;y++)for(x=0;x<N;x++){ if(!_sol(x,y)||_hol(x,y)) continue; _g.fillStyle=((x-C)+(y-C)<-1.5)?'#8a7ea0':'#6a6080'; _g.fillRect(x,y,1,1); } for(y=0;y<N;y++)for(x=0;x<N;x++){ if(_hol(x,y)){ _g.fillStyle='#0b0910'; _g.fillRect(x,y,1,1); } } }catch(e){} _gc.setAttribute('aria-label','Settings'); tSettings.appendChild(_gc); })(); tSettings.title='Settings'; tSettings.onclick=function(){ try{ openSettings(); }catch(e){} };
      function _tabII(btn,src,label){ try{ btn.textContent=''; btn.style.display='flex'; btn.style.flexDirection='column'; btn.style.alignItems='center'; btn.style.justifyContent='center'; btn.style.gap='3px'; btn.style.padding='5px 1px'; var im=document.createElement('img'); im.src=src; im.alt=label; im.style.cssText='width:20px;height:20px;image-rendering:pixelated;display:block;flex:0 0 auto;'; btn.appendChild(im); var s=document.createElement('span'); s.textContent=label; s.style.cssText="font-family:'Press Start 2P',monospace;font-size:5px;letter-spacing:0.3px;line-height:1.2;text-align:center;"; btn.appendChild(s); }catch(e){} }
    ...
    function penPlaceKick(){
      scoring=false; celebrated=false; moving=false; aiming=false; aimStart=null; aimNow=null; netBulge=0; banner=0; hitStop=0; goalFlash=0; goalSlow=0; shotTrail=[]; hitSparks=[]; coin.spin=0; coin.vx=0; coin.vy=0; flickCount=0;
    ...
    function buildExhibition(host){ fmtRefreshers=[]; if(!tutSeen('exh')){ buildTutorialSetup(host); return; } _cfgApply('exhibition'); pitchStepper(host); var cols=mk('div','display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-top:5px;'); var _rS=teamStepper('red','YOU','#a9c94b'); var _bS=teamStepper('blue','CPU','#a9c94b'); cols.appendChild(_rS); cols.appendChild(_bS); function updExhLabels(){ var twoP=(cpuSel!=='cpu'); if(_rS.firstChild) _rS.firstChild.textContent=twoP?'PLAYER 1':'YOU'; if(_bS.firstChild) _bS.firstChild.textContent=twoP?'PLAYER 2':'CPU'; } updExhLabels(); host.appendChild(cols); squadControls(host); var diffWrap=mk('div',''); segRow(host,'OPPONENT',[['off','2 PLAYERS'],['cpu','CPU']],function(){return cpuSel;},function(v){ cpuSel=v; updExhLabels(); diffWrap.style.display=v==='cpu'?'block':'none'; },2); segRow(diffWrap,'DIFFICULTY',[['easy','EASY'],['med','MEDIUM'],['hard','HARD']],function(){return exhLevel;},function(v){exhLevel=v;},3); diffWrap.style.display=cpuSel==='cpu'?'block':'none'; host.appendChild(diffWrap); segRow(host,'GOALS TO WIN',[[3,'3'],[5,'5'],[7,'7']],function(){return exhWin;},function(v){exhWin=v;},3); var play=mk('button','margin-top:6px;width:100%;'+FS(11,'#0b1a0e')+'background:#a9c94b;border:2px solid #e6ff7a;padding:9px;cursor:pointer;','PLAY  ▸'); play.onclick=function(){ teamKits.red=resolveSel('red'); teamKits.blue=resolveSel('blue',teamKits.red); buildBoard(); buildCrowd(); mode='exhibition'; winTarget=exhWin; matchLen=0; aiLevel=exhLevel; var _tutGo=!tutSeen('exh'); if(_tutGo) winTarget=3; /* tutorial match is always first-to-3 */ aiEnabled={red:false,blue:cpuSel==='cpu'}; pre.style.display='none'; newMatch(); try{ showVsIntro(); }catch(e){} if(_tutGo){ try{ setTimeout(function(){ tutStart('exh'); },2300); }catch(e){} } }; host.appendChild(play); } function buildPractice(host){ fmtRefreshers=[]; pitchStepper(host); var cols=mk('div','display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:8px;'); cols.appendChild(teamStepper('red','TEAM 1','#a9c94b')); cols.appendChild(teamStepper('blue','TEAM 2','#a9c94b')); host.appendChild(cols); squadControls(host); var ptWrap=mk('div',''); segRow(host,'MATCH TYPE',[['normal','NORMAL'],['penalty','PENALTIES']],function(){return pracType;},function(v){ pracType=v; ptWrap.style.display=v==='penalty'?'block':'none'; },2); segRow(ptWrap,'BEST OF',[[3,'3 KICKS'],[5,'5 KICKS']],function(){return penBest;},function(v){penBest=v;},2); ptWrap.style.display=pracType==='penalty'?'block':'none'; host.appendChild(ptWrap); var play=mk('button','margin-top:8px;width:100%;'+FS(11,'#0b1a0e')+'background:#a9c94b;border:2px solid #e6ff7a;padding:9px;cursor:pointer;','PLAY  ▸'); play.onclick=function(){ teamKits.red=resolveSel('red'); teamKits.blue=resolveSel('blue',teamKits.red); buildBoard(); buildCrowd(); if(pracType==='penalty'){ aiLevel=exhLevel; pre.style.display='none'; startPenalty(false, exhLevel, penBest); return; } mode='practice'; winTarget=99; matchLen=0; aiLevel=exhLevel; aiEnabled={red:false,blue:false}; pendingAb=null; pre.style.display='none'; newMatch(); }; host.appendChild(play); } function buildRoyaleSetup(host, forceNew){ fmtRefreshers=[]; var _royRun=null; if(!forceNew){ try{ _royRun=spGetRun('royale'); }catch(e){} } host.appendChild(mk('div',FS(9,'#a9c94b')+'text-align:center;margin:6px 0 6px;letter-spacing:1px;','STADIUM ROYALE')); if(_royRun){ var _lm={easy:'EASY',med:'MEDIUM',hard:'HARD'}, _lvl=_lm[_royRun.royaleLevel]||'MEDIUM', _stg=(_royRun.royale&&typeof _royRun.royale.i==='number')?(_royRun.royale.i+1):1, _sn=(_royRun.royale&&_royRun.royale.map===2)?2:1, _rlen=((_sn===2)?ROYALE_ARENAS_2.length:ROYALE_ARENAS_1.length); host.appendChild(mk('div',FS(8,'#a9c94b')+'text-align:center;margin:8px 0 2px;','RUN IN PROGRESS')); host.appendChild(mk('div',FS(7,'#9a8fb0')+'text-align:center;line-height:1.9;margin:2px 0 6px;','SEASON '+_sn+'  •  STADIUM '+_stg+' OF '+_rlen+'  •  '+_lvl)); var resume=mk('button','margin-top:10px;width:100%;'+FS(11,'#0b1a0e')+'background:#a9c94b;border:2px solid #e6ff7a;padding:11px;cursor:pointer;','RESUME ROYALE  ▸'); try{ spSetResume(resume,'royale'); }catch(e){} host.appendChild(resume); var neww=mk('button','margin-top:8px;width:100%;'+FS(9,'#8a7ea0')+'background:#0e0a16;border:2px solid #3a3050;padding:9px;cursor:pointer;','NEW ROYALE'); neww.onclick=function(){ host.innerHTML=''; buildRoyaleSetup(host, true); }; host.appendChild(neww); return; } host.appendChild(mk('div',FS(8,'#9a8fb0')+'margin:2px 0 4px;','YOUR TEAM')); var trow=mk('div','display:flex;justify-content:center;'); trow.appendChild(teamStepper('red','TEAM','#a9c94b')); host.appendChild(trow); if(royMap===2&&!roySeasonUnlocked(2)) royMap=1; if(!royLvlUnlocked(royMap,royaleLevel)) royaleLevel=royTopUnlocked(royMap); var _diffRestyle=segRow(host,'DIFFICULTY',[['easy','EASY'],['med','MEDIUM'],['hard','HARD']],function(){return royaleLevel;},function(v){ royaleLevel=v; },3,function(v){ if(royLvlUnlocked(royMap,v)) return ''; return (v==='med')?'CLEAR EASY TO UNLOCK MEDIUM':'CLEAR MEDIUM TO UNLOCK HARD'; },function(v){ return royDiffsFor(royMap).indexOf(v)>=0; }); host.appendChild(mk('div',FS(7,'#9a8fb0')+'text-align:center;line-height:1.8;margin:10px 0 2px;','Each stadium has a wild twist that hits both sides. Win to advance — one loss ends the whole run. Can you go unbeaten?')); host.appendChild(mk('div',FS(8,'#9a8fb0')+'margin:10px 0 3px;text-align:center;','LADDER')); var mapRow=mk('div','display:flex;gap:6px;justify-content:center;'); var mapBtns=[]; function _s2lock(){ return !roySeasonUnlocked(2); } function restyleMap(){ mapBtns.forEach(function(pr){ var lk=(pr[1]===2&&_s2lock()); var sel=(royMap===pr[1]&&!lk); pr[0].textContent=(lk?'🔒 ':'')+pr[2]; pr[0].style.background=sel?'#a9c94b':'#0e0a16'; pr[0].style.color=sel?'#0b0910':(lk?'#5a5070':'#8a7ea0'); pr[0].style.borderColor=sel?'#a9c94b':(lk?'#2a2438':'#3a3050'); pr[0].style.opacity=lk?'0.6':'1'; }); } [['SEASON 1',1],['SEASON 2',2]].forEach(function(o){ var b=mk('button',SQBTN+'font-size:7px;padding:6px 10px;',o[0]); b.onclick=function(){ if(o[1]===2&&_s2lock()){ try{ _lockMsg('CLEAR SEASON 1 TO UNLOCK SEASON 2'); }catch(e){} return; } royMap=o[1]; if(!royLvlUnlocked(royMap,royaleLevel)) royaleLevel=royTopUnlocked(royMap); restyleMap(); try{ _diffRestyle(); }catch(e){} }; mapBtns.push([b,o[1],o[0]]); mapRow.appendChild(b); }); restyleMap(); host.appendChild(mapRow); var start=mk('button','margin-top:10px;width:100%;'+FS(11,'#0b1a0e')+'background:#a9c94b;border:2px solid #e6ff7a;padding:11px;cursor:pointer;','START ROYALE  ▸'); start.onclick=function(){ teamKits.red=resolveSel('red'); buildCrowd(); startRoyale(PRESETS.indexOf(teamKits.red)); }; host.appendChild(start); } function buildTourSetup(host){ fmtRefreshers=[]; _cfgApply('tournament'); host.appendChild(mk('div',FS(9,'#a9c94b')+'text-align:center;margin:6px 0 6px;letter-spacing:1px;','WORLD CUP')); host.appendChild(mk('div',FS(8,'#9a8fb0')+'margin:2px 0 4px;','YOUR TEAM')); var trow=mk('div','display:flex;justify-content:center;'); trow.appendChild(teamStepper('red','TEAM','#a9c94b')); host.appendChild(trow); if(!cupSizeUnlocked(tourSize)) tourSize=cupTopSize();
    ...
    function showBracket(advanced){ try{spSaveRun();}catch(e){}
      ...
      ;(function(){ if(!advanced) return; var _lc=tour.rounds.length-1, _pj=curRound().indexOf(tour.playerPreset); if(_pj<0||_lc<1) return; var _np=posAt(_lc,_pj), _prr=tour.rounds[_lc-1], _ppj=_prr?_prr.indexOf(tour.playerPreset):-1; if(_ppj<0) return; var _pp=posAt(_lc-1,_ppj); var snap; try{ snap=g.getImageData(0,0,SZ,BH); }catch(e){ return; } var t0=null; try{ if(typeof haptic==='function') haptic(18); }catch(e){} function anim(ts){ if(!cv||!el('ns_pre')) return; if(t0===null)t0=ts; var p=Math.min(1,(ts-t0)/760), e=1-Math.pow(1-p,3); try{ g.putImageData(snap,0,0); }catch(e2){ return; } var ax=_pp.x+(_np.x-_pp.x)*e, ay=_pp.y+(_np.y-_pp.y)*e; g.save(); var rad=g.createRadialGradient(ax,ay,0,ax,ay,10); rad.addColorStop(0,'rgba(255,216,74,0.95)'); rad.addColorStop(1,'rgba(255,216,74,0)'); g.fillStyle=rad; g.beginPath(); g.arc(ax,ay,10,0,6.283); g.fill(); g.fillStyle='#fff3c4'; g.beginPath(); g.arc(ax,ay,2.6,0,6.283); g.fill(); g.restore(); if(p>0.8){ var pp=(p-0.8)/0.2; g.save(); g.globalAlpha=1-pp; g.strokeStyle='#ffd84a'; g.lineWidth=2; g.beginPath(); g.arc(_np.x,_np.y,4+pp*11,0,6.283); g.stroke(); g.restore(); } if(p<1){ requestAnimationFrame(anim); } else { try{ g.putImageData(snap,0,0); }catch(e3){} } } requestAnimationFrame(anim); })();
      function showCountryCard(idx){ var p=PRESETS[idx]; var old=el('ns_ccard'); if(old) old.remove(); var ov=mk('div','position:fixed;inset:0;z-index:40;display:flex;align-items:center;justify-content:center;background:rgba(6,4,10,0.78);padding:14px;box-sizing:border-box;'); ov.id='ns_ccard'; var panel=mk('div','width:100%;max-width:300px;background:linear-gradient(#1a1330,#0e0a18);border:2px solid #4a3a5e;border-radius:10px;padding:14px 12px;box-shadow:0 6px 20px rgba(0,0,0,0.6);'); try{ var _bk=p.name.toLowerCase(); if(typeof NS_VSBG!=='undefined'&&NS_VSBG[_bk]&&NS_VSBG[_bk].complete&&NS_VSBG[_bk].naturalWidth){ panel.style.background='linear-gradient(180deg,rgba(14,9,20,0.5),rgba(14,9,20,0.32) 45%,rgba(14,9,20,0.62)),url('+NS_VSBG[_bk].src+') center/cover'; } }catch(e){} var head=mk('div','display:flex;align-items:center;justify-content:center;gap:8px;margin-bottom:8px;'); var fc=document.createElement('canvas'); fc.width=24; fc.height=16; fc.style.cssText='width:26px;height:18px;image-rendering:pixelated;border:1px solid rgba(0,0,0,0.5);'; var fg=fc.getContext('2d'); fg.imageSmoothingEnabled=false; paintPattern(fg,0,0,24,16,p.kit); head.appendChild(fc); head.appendChild(mk('div',FS(10,'#f4e9c8'),p.name.toUpperCase())); panel.appendChild(head); var bd=BOARDS[COUNTRY_PITCH[p.name]]||BOARDS.grass; panel.appendChild(mk('div',FS(7,'#a9c94b')+'text-align:center;margin-bottom:7px;','HOME PITCH: '+bd.name)); var _isP=(idx===tour.playerPreset); var fmc=document.createElement('canvas'); fmc.width=W; fmc.height=H; fmc.style.cssText='width:132px;height:auto;image-rendering:pixelated;display:block;margin:0 auto 8px;border:2px solid #4a3a5e;border-radius:4px;'; var _oppStyle=(p.name===PRESETS[tour.playerPreset].name)?'away':'flag'; function paintFmc(){ try{ renderCountryField(fmc.getContext('2d'),p.name,p.kit,(_isP?((typeof kitStyle!=='undefined'&&kitStyle.red)?kitStyle.red:'jersey'):_oppStyle),(_isP?'red':'blue')); }catch(e){} } paintFmc(); panel.appendChild(mk('div',FS(7,'#a9c94b')+'text-align:center;margin-bottom:5px;','FORMATION')); panel.appendChild(fmc); function abRow(id){ var t=TACTIC_MAP[id]; if(!t) return; var row=mk('div','display:flex;align-items:center;gap:9px;margin-bottom:8px;'); var ic=mk('div','width:34px;height:34px;flex:0 0 auto;display:flex;align-items:center;justify-content:center;font-size:19px;'); if(ICON_IMG[id]&&ICON_IMG[id].ok){ var im=document.createElement('img'); im.src=ICON_SRC[id]; im.style.cssText='width:100%;height:100%;object-fit:contain;image-rendering:pixelated;'; ic.appendChild(im); } else { ic.textContent=t.icon; } var tx=mk('div','flex:1;'); tx.appendChild(mk('div',FS(7,'#a9c94b'),t.name)); tx.appendChild(mk('div','font-size:10px;line-height:1.45;color:#b9add0;',t.desc)); row.appendChild(ic); row.appendChild(tx); panel.appendChild(row); } if(idx===tour.playerPreset){ panel.appendChild(mk('div',FS(7,'#a9c94b')+'text-align:center;margin:2px 0 5px;','YOUR KIT')); var ksRow=mk('div','display:flex;gap:7px;justify-content:center;margin-bottom:10px;'); var ksItems=[]; ['flag','jersey','away'].forEach(function(stl){ var kb=mk('button','padding:2px;cursor:pointer;border:2px solid #3a3050;border-radius:6px;background:#0e0a16;line-height:0;'); var cvv=document.createElement('canvas'); cvv.width=30; cvv.height=30; cvv.style.cssText='width:30px;height:30px;image-rendering:pixelated;display:block;'; kb.appendChild(cvv); kb.onclick=function(e){ if(e&&e.stopPropagation)e.stopPropagation(); kitStyle.red=stl; paintKsC(); paintFmc(); }; ksItems.push({b:kb,cv:cvv,style:stl}); ksRow.appendChild(kb); }); function paintKsC(){ var cur=(kitStyle&&kitStyle.red)?kitStyle.red:'jersey'; ksItems.forEach(function(it){ var g2=it.cv.getContext('2d'); g2.imageSmoothingEnabled=false; g2.clearRect(0,0,30,30); try{ paintNail(g2,15,15,12,p.kit,false,resolveKit(p.kit,it.style)); }catch(e){} var on=cur===it.style; it.b.style.borderColor=on?'#a9c94b':'#3a3050'; it.b.style.background=on?'rgba(198,232,74,0.12)':'#0e0a16'; }); } paintKsC(); panel.appendChild(ksRow); panel.appendChild(mk('div',FS(7,'#a9c94b')+'text-align:center;margin:2px 0 6px;','YOUR ABILITIES')); var own=(tour.playerAb||[]).slice(0,3); if(own.length){ own.forEach(abRow); panel.appendChild(mk('div','font-size:10px;line-height:1.55;color:#b9add0;text-align:center;margin-top:2px;','Carried through the cup. Score with full slots to swap one.')); } else { panel.appendChild(mk('div','font-size:11px;line-height:1.6;color:#b9add0;text-align:center;','No abilities yet — win one on every goal, and they carry between matches.')); } } else { var cnt=tour.level==='easy'?1:tour.level==='med'?2:3; (COUNTRY_AB[p.name]||[]).slice(0,cnt).forEach(abRow); } ov.appendChild(panel); ov.addEventListener('click',function(e){ if(e.target===ov) ov.remove(); }); document.body.appendChild(ov); }
    ...
    function teamStepper(side,label,accent){
      ...
      function render(){ var p=curPreset(side), g=sw.getContext('2d'); try{ var _tc=sel[side].rand?'#a9c94b':((p&&p.primary)||accent); box.style.borderColor='#4a3a5e'; }catch(e){} g.imageSmoothingEnabled=false; g.clearRect(0,0,30,30); if(sel[side].rand){ g.fillStyle='rgba(0,0,0,0.30)'; g.beginPath(); g.arc(16,16.5,13,0,Math.PI*2); g.fill(); g.beginPath(); g.arc(15,15,13,0,Math.PI*2); g.fillStyle='#1a1330'; g.fill(); g.lineWidth=1.2; g.strokeStyle='rgba(0,0,0,0.6)'; g.stroke(); g.fillStyle=accent; g.font="13px 'Press Start 2P',monospace"; g.textAlign='center'; g.textBaseline='middle'; g.fillText('?',15,15.5); nm.textContent='RANDOM'; } else { try{ flagCoin(g,15,15,13,p.kit,false); }catch(e){ paintPattern(g,2,2,26,26,p.kit); } nm.textContent=p.abbr; } if(typeof paintKs==='function') paintKs(); refreshPitch(); if(_diceBtn){ var _on=sel[side].rand; _diceBtn.style.background=_on?accent:'#0e0a16'; _diceBtn.style.color=_on?'#0b0910':'#c9bce0'; _diceBtn.style.borderColor=_on?accent:'#3a3050'; } }
