    ...
    function segRow(host,label,opts,getVal,setVal,cols){
      ...
      const btns=[];
      function restyle(){ btns.forEach(([b,v])=>{ const s=getVal()===v; b.style.background=s?'#a9c94b':'#0e0a16'; b.style.color=s?'#0b0910':'#8a7ea0'; b.style.borderColor=s?'#a9c94b':'#3a3050'; }); }
      opts.forEach(([v,l])=>{ const b=mk('button',SQBTN+'font-size:7px;padding:4px;text-align:center;',l); b.onclick=()=>{ setVal(v); restyle(); }; btns.push([b,v]); row.appendChild(b); });
