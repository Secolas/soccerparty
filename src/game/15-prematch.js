    ...
    function penResolve(kind){
      ...
      if(dec){
        ...
        setTimeout(function(){ if(!pen) return; penGameOver(dec); }, 1400);
      } else {
    ...
    function drawPenaltyHUD(){
      ...
      function prow(side, cy){ var arr=pen.history[side], seg, total;
        ...
        for(var i=0;i<total;i++){ var x=x0+i*gap; ctx.beginPath(); ctx.arc(x,cy,dotR,0,Math.PI*2); if(i<seg.length){ ctx.fillStyle=(seg[i]==='G')?'#5dff5d':'#ff5a4a'; ctx.fill(); } else { ctx.strokeStyle='rgba(255,255,255,0.55)'; ctx.lineWidth=1; ctx.stroke(); } } }
    ...
    function teamStepper(side,label,accent){
      const box=mk('div',"background:#14101e;border:2px solid "+accent+";padding:5px 2px 5px;display:flex;flex-direction:column;align-items:center;gap:3px;overflow:hidden;");
      ...
      function render(){ var p=curPreset(side), g=sw.getContext('2d'); try{ var _tc=sel[side].rand?'#c6e84a':((p&&p.primary)||accent); box.style.borderColor=_tc; if(box.firstChild) box.firstChild.style.color=_tc; }catch(e){} g.imageSmoothingEnabled=false; g.clearRect(0,0,32,24); if(sel[side].rand){ g.fillStyle='#1a1330'; g.fillRect(0,0,32,24); g.fillStyle=accent; g.font="13px 'Press Start 2P',monospace"; g.textAlign='center'; g.textBaseline='middle'; g.fillText('?',16,13); nm.textContent='RANDOM'; } else { paintPattern(g,0,0,32,24,p.kit); nm.textContent=p.abbr; } g.strokeStyle='rgba(0,0,0,0.5)'; g.strokeRect(0.5,0.5,31,23); if(typeof paintKs==='function') paintKs(); refreshPitch(); if(_diceBtn){ var _on=sel[side].rand; _diceBtn.style.background=_on?accent:'#0e0a16'; _diceBtn.style.color=_on?'#0b0910':'#c9bce0'; _diceBtn.style.borderColor=_on?accent:'#3a3050'; } }
      ...
      buildFmt(); box.appendChild(fmtRow); if(typeof fmtRefreshers!=='undefined') fmtRefreshers.push(buildFmt);
      render();
      return box;
    }
