...
const ASSETS = [
  ...
  { file: "icon-app.png",         size: 512, keyOut: false, prompt: `A square mobile app icon in chunky retro 16-bit PIXEL ART with clearly visible large square pixels and bold outlines. A single classic black-and-white soccer ball, large and centred, filling most of the frame, on a vibrant lime-green background with a subtle darker grass-stripe pitch feel; a few small colourful confetti pieces (party) popping around the ball; high contrast, clean and simple so it reads at tiny sizes; no text, no lettering, fills the whole square edge to edge, no border.` },
