...
const ASSETS = [
  ...
  { file: "roymap.png",            size: 512, keyOut: false, prompt: `A top-down illustrated adventure map, chunky retro 16-bit PIXEL ART, full-bleed edge to edge. The map is divided into NINE horizontal TERRAIN BANDS stacked from BOTTOM to TOP, and ONE continuous winding dirt trail snakes upward crossing every band in turn, from the bottom edge up to the top. From BOTTOM to TOP the bands are, in this exact order: (1) frozen ICE and snow with a blue frozen pond and ice blocks; (2) golden SAVANNA grassland with flat-topped acacia trees; (3) a grey COBBLESTONE TOWN of little paved stone houses; (4) an orange DESERT of sand dunes; (5) a dark NEON NIGHT-CITY of tiny glowing skyscrapers with pink and cyan lights; (6) a foggy grey STREET with drifting mist and lamp posts; (7) a wooden WAREHOUSE district of timber sheds and crates; (8) a grey STONE FORTRESS wall with a castle; (9) at the very top a lush green GRASS football pitch with a big golden TROPHY stadium. Each band is a clearly different biome, the single trail visibly weaving up through all nine in order. Keep the trail itself clear - absolutely NO text, NO numbers, NO dots, NO waypoint markers, NO flags on the trail. Cohesive palette, clearly visible large square pixels, charming. ${SCENE}` },
];
