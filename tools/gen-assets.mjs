...
const ASSETS = [
  ...
  // Warehouse spider — 4-frame TOP-DOWN walk cycle (rotated by heading in-game).
  { file: "sprite-spider", frames: 4, size: 48, deFringe: true, prompt: `A 4-frame pixel-art sprite sheet: FOUR copies of the SAME small pale grey-white top-down spider in ONE horizontal row, identical spider viewed from directly ABOVE (birds eye, looking straight down), a round fuzzy body in the middle with eight legs splayed out around it, tiny dark eyes. Across the four frames the eight legs cycle in a walking motion (legs bend and shift) while the body stays the same size and centered in each frame. Clear even vertical gaps separating the four frames left to right. ABSOLUTELY NO web, NO silk, NO ground, NO dividing lines or frame borders, each spider floats alone surrounded by wide empty magenta space. ${ICON}` },
