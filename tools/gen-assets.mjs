...
const ASSETS = [
  ...
  // Animated mud — 8-frame loop of a top-down thick muddy bog slowly churning.
  { file: "sprite-mud", frames: 8, size: 64, keyOut: false, prompt: `A horizontal sprite sheet of EXACTLY 8 equal frames in a single row, side by side, showing the SAME square patch of top-down thick wet MUD slowly bubbling and glistening through ONE smooth seamless loop. From frame 1 to frame 8 a few mud bubbles rise and pop and the wet highlights shift a little so playing them in order looks like slowly churning sticky mud, and frame 8 flows back into frame 1. Each frame is full-bleed mud filling the whole square, seamless, dark brown and muddy ochre tones, chunky 16-bit pixel art, NO water, NO borders or gaps or lines between frames, no text. ${SCENE}` },
