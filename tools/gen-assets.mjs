...
const ASSETS = [
  ...
  { file: "icon-jelly.png",  size: 64,  prompt: `A wobbly bright purple jelly trampoline pad / bouncy dome, glossy highlight, chunky retro pixel-art power-up icon. ${ICON}` },
  { file: "icon-caramel.png",  size: 64,  prompt: `A gooey brown caramel toffee blob puddle, sticky and glossy, chunky retro pixel-art power-up icon. ${ICON}` },
  { file: "icon-gumball.png",  size: 64,  prompt: `A shiny red round gumball candy with a white highlight, chunky retro pixel-art power-up icon. ${ICON}` },
  { file: "icon-gust.png",  size: 64,  prompt: `Curved white wind-gust swirl streaks blowing to one side, airy motion lines, chunky retro pixel-art power-up icon. ${ICON}` },
  { file: "icon-rainpuddle.png",  size: 64,  prompt: `A blue rain puddle with concentric ripple rings and a raindrop splash, chunky retro pixel-art power-up icon. ${ICON}` },
  { file: "icon-lightning.png",  size: 64,  prompt: `A bright yellow jagged lightning bolt striking down, electric glow, chunky retro pixel-art power-up icon. ${ICON}` },
  { file: "icon-current.png",  size: 64,  prompt: `Two wavy blue water current arrows flowing sideways, underwater stream, chunky retro pixel-art power-up icon. ${ICON}` },
  { file: "icon-bubbles.png",  size: 64,  prompt: `A rising column of round teal water bubbles of different sizes, chunky retro pixel-art power-up icon. ${ICON}` },
  { file: "icon-whirlpool.png",  size: 64,  prompt: `A blue spiral whirlpool vortex swirling inward, foam highlights, chunky retro pixel-art power-up icon. ${ICON}` },
  { file: "icon-gravity.png",  size: 64,  prompt: `A glowing purple gravity plate disc with arrows pulling inward toward the centre, chunky retro pixel-art power-up icon. ${ICON}` },
  { file: "icon-asteroid.png",  size: 64,  prompt: `A chunky grey pitted space rock asteroid with craters, chunky retro pixel-art power-up icon. ${ICON}` },
  { file: "icon-blackhole.png",  size: 64,  prompt: `A black hole: a black core ringed by a swirling purple-blue accretion glow, chunky retro pixel-art power-up icon. ${ICON}` },
  { file: "icon-roulette.png",  size: 64,  prompt: `A red-and-black roulette wheel seen from above with a white ball in a pocket, chunky retro pixel-art power-up icon. ${ICON}` },
  { file: "icon-dice.png",  size: 64,  prompt: `A single white six-sided die showing five pips, 3D corner view, chunky retro pixel-art power-up icon. ${ICON}` },
  { file: "icon-numbox.png",  size: 64,  prompt: `A wooden crate stamped with a bold number, casino number box, chunky retro pixel-art power-up icon. ${ICON}` },
  { file: "icon-track.png",  size: 64,  prompt: `A looping curved orange hot-wheels style track tube segment, chunky retro pixel-art power-up icon. ${ICON}` },
  { file: "icon-halfpipe.png",  size: 64,  prompt: `A grey concrete skate half-pipe ramp curve seen from the side, U shape, chunky retro pixel-art power-up icon. ${ICON}` },
  { file: "icon-bump.png",  size: 64,  prompt: `A small raised skatepark bump / speed hump with a dashed centre line, chunky retro pixel-art power-up icon. ${ICON}` },
  { file: "icon-vine.png",  size: 64,  prompt: `A hanging green jungle vine with leaves, curled at the bottom, chunky retro pixel-art power-up icon. ${ICON}` },
  { file: "icon-banana.png",  size: 64,  prompt: `A ripe yellow banana lying flat, glossy peel, chunky retro pixel-art power-up icon. ${ICON}` },
  { file: "icon-log.png",  size: 64,  prompt: `A brown wooden rolling log / bark barrel with end rings, chunky retro pixel-art power-up icon. ${ICON}` },
  { file: "icon-wave.png",  size: 64,  prompt: `A curling blue ocean wave crest with white foam, chunky retro pixel-art power-up icon. ${ICON}` },
  { file: "icon-crab.png",  size: 64,  prompt: `A little orange crab seen from above with two claws and eyes, chunky retro pixel-art power-up icon. ${ICON}` },
  { file: "icon-beachball.png",  size: 64,  prompt: `A classic colourful segmented red-yellow-blue-white beach ball, chunky retro pixel-art power-up icon. ${ICON}` },
  ...
  { file: "icon-champion.png", size: 64,  prompt: `A shiny golden winners trophy cup in the style of the 🏆 emoji: a wide two-handled gold cup on a small base, bright yellow-gold with lighter gold highlights and a subtle sparkle, a victorious champion vibe, ABSOLUTELY NO text letters and no person, ONLY the trophy, thick dark outline, chunky bold flat shapes, big and simple, centered, victory icon. ${ICON}` },
  ...
  // Season 2's journey map. Same illustrated treatment as roymap.png so the two
  // seasons feel like one atlas, but an ARCHIPELAGO instead of a continent —
  // Season 2's nine stadiums are far too varied to share one landmass, and an
  // island chain reads as a clear break from Season 1.
  { file: "roymap2.png",           size: 512, keyOut: false, prompt: `A beautiful hand-illustrated fantasy ADVENTURE MAP, richly detailed 16-bit PIXEL ART, FULL BLEED, land running to all four edges. NO border, NO frame, NO panel edges, NO dividing lines, NO margin, NO text, NO letters, NO numbers, NO labels. ONE single connected continent seen from above, with NINE neighbouring regions blending into each other through forest, hill and river - never separate tiles or islands. Each region holds ONE clearly visible RECTANGULAR FOOTBALL PITCH with white markings and a small goal at each end, made of that region's material, and each pitch is COVERED IN ITS OWN OBSTACLES so you can see what the stadium does. BOTTOM ROW, left to right: (1) a PINK CANDY land - a glossy pink pitch strewn with wobbly JELLY PADS, puddles of sticky brown CARAMEL and big rolling GUMBALLS; (2) a THUNDERSTORM ridge - a wet dark slate pitch under black clouds, with swirling wind, silver RAIN PUDDLES and forked LIGHTNING striking the pitch; (3) a giant glass AQUARIUM tank standing in the sea - a pale blue-green pitch inside, with swirling CURRENTS, columns of BUBBLES erupting from the floor and a BIG FISH swimming past the glass. MIDDLE ROW, left to right: (4) a starry ORBIT LAB - a dark metal space-station deck pitch studded with glowing MAGNET PLATES, floating debris and a ringed planet overhead; (5) a red and gold CASINO - a deep red card-table pitch with an ENORMOUS SPINNING ROULETTE WHEEL set into the middle of the pitch and several GIANT WHITE DICE scattered across it, ringed by card-house buildings and gold chips; (6) a concrete SKATE PARK - a pale grey pitch flanked by curved half-pipe RAMPS on both sides and metal GRIND RAILS crossing it. TOP ROW, left to right: (7) JUNGLE RUINS - a mossy grey stone pitch inside a vine-covered temple, with long swinging VINES hanging over it; (8) a SANDY SHORE - a pale golden sand pitch beside a bright blue lake, with rolling TIDE WAVES, striped BEACH BALLS and little red CRABS on it; (9) top-right, a majestic floodlit BLUE STADIUM holding a lush green striped GRASS pitch, with a huge GOLDEN TROPHY above it. Warm cohesive palette, crisp detailed pixel art, charming and epic. ${SCENE}` },
];
