...
const ASSETS = [
  ...
  { file: "roymap.png",            size: 512, keyOut: false, prompt: `A top-down illustrated fantasy ADVENTURE MAP in chunky retro 16-bit PIXEL ART, like an old board-game or Pilgrim's Progress quest map, full-bleed edge to edge. A single long winding dirt trail snakes from a small soccer village at the BOTTOM up to a grand golden trophy stadium at the TOP. Along the way the trail passes distinct themed biomes: a snowy ice patch, a green savanna with acacia trees, a little cobblestone town, sandy desert dunes, a glowing neon night-city block, a foggy dark alley, a wooden warehouse, and a grey stone fortress castle. Soft parchment and grass tones, rivers, little trees and hills, soccer motifs (goalposts, a ball or two). Leave the winding trail itself CLEAR and open — NO text, NO labels, NO numbered markers, NO dots or flags on the path. Cohesive warm palette, clearly visible large square pixels, decorative and charming. ${SCENE}` },
];
